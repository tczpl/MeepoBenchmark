...
for member_cnt in [4]:
    for shard_cnt in shards:
        ...
        avg_epoch_number = util.get_epochnumber(logdir)
        to1.append(avg_epoch_number)
        print(shard_cnt, member_cnt, avg_epoch_number)

        logdir = "meepo128/"+str(shard_cnt)+"x"+str(member_cnt)+"_shop2_"
        avg_epoch_number = util.get_epochnumber(logdir)
        to2.append(avg_epoch_number)
        print(shard_cnt, member_cnt, avg_epoch_number)
...
plt.ylabel('Number of Cross-epochs', fontsize=20)
